...
Reviewed schema changes produce append-only files in `../migrations/` for deployment history. Generated `packages/supabase/src/generated/database.types.ts` is a projection of the applied schema. Tables exposed through the Data API require explicit grants and RLS policies appropriate to their resource and ownership model.
