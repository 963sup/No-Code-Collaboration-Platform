...
4. Generate a timestamped Draft migration from the declared state, then review all SQL. Add reviewed DML or unsupported PostgreSQL objects explicitly rather than assuming the diff engine captured them.
...
8. Keep migrations, schema files, pgTAP attack-path tests, and directly affected adapters/contracts in the same reviewed change. Stop when the observable contract is proven.
