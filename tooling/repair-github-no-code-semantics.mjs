...
for (const [before, after] of [
  ["and (select private.has_repository_capability(repository_id, 'resource.update'))\n  and (select private.user_can_view_repository(user_id, repository_id))", "and (select private.has_repository_capability(repository_id, 'issue.manage'))\n  and (select private.user_can_view_repository(user_id, repository_id))"],
  ["= 'assign'\n  and (select private.has_repository_capability(repository_id, 'resource.update'))", "= 'assign'\n  and (select private.has_repository_capability(repository_id, 'issue.manage'))"],
  ["= 'label'\n  and (select auth.uid()) = applied_by\n  and (select private.has_repository_capability(repository_id, 'resource.update'))", "= 'label'\n  and (select auth.uid()) = applied_by\n  and (select private.has_repository_capability(repository_id, 'issue.manage'))"],
  ["= 'label'\n  and (select private.has_repository_capability(repository_id, 'resource.update'))", "= 'label'\n  and (select private.has_repository_capability(repository_id, 'issue.manage'))"],
  ["= 'comment'\n  and (select auth.uid()) = created_by\n  and (select private.has_repository_capability(repository_id, 'resource.create'))", "= 'comment'\n  and (select auth.uid()) = created_by\n  and (select private.has_repository_capability(repository_id, 'issue.comment'))"]
]) replaceOnce(rls, before, after);
replaceOnce(
  rls,
