...
describe('SupabaseIdentityProvider', () => {
  ...
  it('exchanges a PKCE recovery proof for a signed recovery session', async () => {
    ...
    const result = provider.verifyPasswordRecovery('pkce_opaque-recovery-token-hash');
  ...
  it('rejects non-PKCE recovery proofs before provider exchange', async () => {
    const client = {
      auth: {}
    } as unknown as SupabaseClient<Database>;
