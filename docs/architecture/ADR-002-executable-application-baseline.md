...
Implement the first deployable system as a TypeScript modular monolith with one Next.js application and four coarse-grained packages:

- `apps/web` — delivery and composition root;
- `packages/domain` — business truth;
- `packages/application` — use-case orchestration and ports;
- `packages/supabase` — infrastructure adapters and generated database projection;
...
Use Supabase declarative schemas as current database truth, generated migrations as reviewed deployment history, generated TypeScript types as a projection inside `packages/supabase`, and PostgreSQL RLS as database enforcement.
...
- Browser/server clients remain request-scoped in the web composition root.
- Database rows cannot leak into domain or UI packages.
...
- Architecture checks reject reversed workspace dependencies and generated-type leakage.
