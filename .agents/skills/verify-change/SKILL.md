...
2. Run `pnpm codex:check` when Codex configuration, agents, skills, hooks, or scoped instructions change.
3. Run `pnpm verify:fast` after normal TypeScript or configuration changes when dependencies are installed.
4. Add `pnpm verify:full` when exports, dependencies, entry points, or dead-code reachability may have changed.
